...
### Mean CVs by set (non-normalize)
```{r}
Set_levels <- levels(phago.assay.sum$Set)
for (x in Set_levels) {
        hist(phago.assay.sum$cv_flu[phago.assay.sum$Set==x],
             xlab = paste("Set = ",x),
...
> I recommend not combining the sets. Set 2 has lower CVs (although still quite high overall).
